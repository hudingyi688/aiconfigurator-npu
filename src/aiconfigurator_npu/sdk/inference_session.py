...
from aiconfigurator_npu.sdk import common, config, models, perf_database
from aiconfigurator_npu.sdk.backends.base_backend import BaseBackend
from aiconfigurator_npu.sdk.inference_summary import InferenceSummary
from aiconfigurator_npu.sdk.picking import (
...
from aiconfigurator_npu.sdk.utils import enumerate_ttft_tpot_constraints
...
class DisaggInferenceSession:
    ...
    def _pick_autoscale(
        ...
        """
        from aiconfigurator_npu.sdk.picking import pick_autoscale
    ...
    def find_best_disagg_result_under_constraints(
        ...
        # drop_duplicates fails when columns contain unhashable types (e.g. numpy.ndarray).
        # Only deduplicate on hashable (scalar) columns.
        import numpy as np
        hashable_cols = [c for c in disagg_summary_df.columns if disagg_summary_df[c].apply(lambda x: not isinstance(x, np.ndarray)).all()]
        disagg_summary_df = disagg_summary_df.drop_duplicates(subset=hashable_cols if hashable_cols else None, ignore_index=True)
